...
// You can write your code in this editor
		...
		for(var i = 0; i < _arrayLen; i++)
		{
			...
			if(global.levelSelectArray[i].compLev1 <= global.levelSelectArray[i].highScore[1])
			{
				global.levelSelectArray[i].completionLevel = 1;
				
				if(global.levelSelectArray[i].compLev2 <= global.levelSelectArray[i].highScore[1])
				{
					global.levelSelectArray[i].completionLevel = 2;
					
					//Switch needed as we want to check for lower time, or higher score
					switch(global.levelSelectArray[i].goalType)
					{
						case GOALTYPE.GOAL :
